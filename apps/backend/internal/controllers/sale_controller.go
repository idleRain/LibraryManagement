...
// ClearCart 清空购物车
func (sc *SaleController) ClearCart(c *gin.Context) {
        sessionID := c.GetHeader("X-Session-ID")
        userID, _ := c.Get("user_id")

        query := sc.db.Where("1 = 1")
        if userID != nil {
                query = query.Where("user_id = ?", userID)
        } else if sessionID != "" {
                query = query.Where("session_id = ?", sessionID)
        ...
        if err := query.Delete(&models.Cart{}).Error; err != nil {
                utils.ServerError(c, "清空失败")
                return
        }

        utils.SuccessWithMsg(c, "清空成功", nil)
}

// UpdateCartQuantity 更新购物车数量
func (sc *SaleController) UpdateCartQuantity(c *gin.Context) {
        var req struct {
                ID       int `json:"id" binding:"required"`
                Quantity int `json:"quantity" binding:"required,min=1"`
        }
        ...
        if err := sc.db.First(&cart, req.ID).Error; err != nil {
        ...
        // 检查库存
        var stock models.Stock
        if err := sc.db.Where("book_id = ?", cart.BookID).First(&stock).Error; err != nil {
                utils.Error(c, 400, "图书库存不存在")
                return
        }
        ...
        if err := sc.db.Save(&cart).Error; err != nil {
                utils.ServerError(c, "更新失败")
                return
        }

        utils.SuccessWithMsg(c, "更新成功", cart)
}
